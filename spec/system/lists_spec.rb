   ...
   describe '投稿画面のテスト' do
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        ...
        expect(page).to have_current_path book_path(List.last)
      end
    end
   end
   ...
   describe '編集画面のテスト' do
    ...
    context '表示の確認' do
      it '編集前のタイトルと本文がフォームに表示(セット)されている' do
        expect(page).to have_filed 'list[title]', with: list.title
        expect(page).to have_filed 'list[body]', with: list.body
      end
      it '保存ボタンが表示される' do
